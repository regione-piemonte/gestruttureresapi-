...
******************************************************************************/
package it.csi.gescovid.gestruttureresapi.filter;
...
import it.csi.gescovid.gestruttureresapi.util.Constants;
...
public class IrideIdAdapterFilter implements Filter {
		...
		if (hreq.getSession().getAttribute(IRIDE_ID_SESSIONATTR) == null) {
			...
			if (marker != null) {
				try {
					...
					it.csi.gescovid.gestruttureresapi.dto.test.UserInfo userInfo = new it.csi.gescovid.gestruttureresapi.dto.test.UserInfo();
	...
	public String getToken(HttpServletRequest httpreq) {
		...
		if (marker == null && devmode) {
            ...
            // se la decodifica non funzionerebbe comunque sempre meglio restituire 
